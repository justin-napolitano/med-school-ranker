...
def build_site_payload(site_mode: str = SITE_MODE_LOCAL_FULL) -> dict[str, object]:
    ...
    for school in school_master:
        ...
        item = {
            ...
            "ranking": public_row(rankings_by_school.get(school_id, {})) if publish_safe else rankings_by_school.get(school_id, {}),
...
function rankingFilters() {
  ...
  const tiers = [...new Set(rows('schools').map(s => s.ranking.dynamic_tier).filter(Boolean))].sort();
  const buckets = [...new Set(rows('schools').map(s => s.ranking.suggested_funnel_bucket).filter(Boolean))].sort();
...
function renderRankingTable() {
  ...
  const columns = [
    ...
    {key:'derived.aamc_acceptance_rate_band', label:'AAMC Band', render:s=>missing(s.derived.aamc_acceptance_rate_band)},
  ];
  if (ADMIN_ENABLED) {
    columns.push(
