...
def test_phase2a_source_outputs_have_expected_counts():
    ...
    assert len(admissions_stats_rows) == 196 + len(aacom_rows)
    ...
    assert report[("admissions_stats", "canonical_rows")]["count"] == "190"
...
def test_admissions_stats_have_quality_and_aamc_bands():
    ...
    assert any(row["data_confidence"].startswith("assumed_match_") for row in rows)
